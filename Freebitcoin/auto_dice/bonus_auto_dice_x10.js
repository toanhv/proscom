...
rollDice = function() {
	...
	if (!$('#bonus_account_wager').text() && parseFloat($('#bonus_account_wager').text()) > 0) {
		throw new Error('Stop game');
	}
	
	if (!$('#bonus_account_balance').text() && parseFloat($('#bonus_account_balance').text()) > 0) {
		throw new Error('Stop game');
	}	
	...
	if ($('#double_your_btc_bet_lose').html() != '') {
		...
		if (loseCount > 50 && $('.maxheight').val() < 30) {
			$('.maxheight').val(30);
		}
		if (loseCount > 60 && $('.maxheight').val() < 35) {
			$('.maxheight').val(35);
		}
		if (loseCount > 70) {
			$('.maxheight').val(40);
