...
(function () {
    ...
    function multiply_2() {
        if (($('#balance').text() / _startBalance) >= _maxProfitPercent)
            stopGame();
    ...
    $("#double_your_btc_bet_win").bind("DOMSubtreeModified", function (a) {
        if ($(a.currentTarget).is(':contains("win")')) {
            ...
            if (iHaveEnoughMoni()) {
                ...
                if ($(".wuynh-lo").val(d), $(".an-lo").val(e), $(".win-next").val(f + 1), g >= j ? ($(".check-lose").val(h + 1), multiply_2()) : ($(".win-dupbo").val(0), reset()), g >= j && h >= 1 && ($(".win-next").val(0), $(".check-lose").val(0), $(".check-win").val(0), $(".win-dupbo").val(0), multiply(), reset()), f >= j && i >= 1 && ($(".win-next").val(0), $(".check-lose").val(0), $(".check-win").val(0), $(".win-dupbo").val(0), multiply(), reset()), stopped)
                    return stopped = !1, !1;
            }
            setTimeout(function () {
                var a = Math.floor(2 * Math.random() + 1);
